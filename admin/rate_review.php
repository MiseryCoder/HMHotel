...
if (isset($_GET['seen'])) {
    ...
    if ($frm_data['seen'] == 'all') {
        ...
        $query = "UPDATE `rating_review` SET `seen`=? WHERE `rating_id`=?";
...
if (isset($_GET['del'])) {
    ...
    if ($frm_data['del'] == 'all') {
        ...
        $query = "DELETE FROM `rating_review` WHERE `rating_id`=?";
        $values = [$frm_data['del']];
        if (update($query, $values, 'i')) {
    ...
    <div class="container-fluid" id="main-content">
        <div class="row">
            <div class="col-lg-10 ms-auto p-4 overflow-hidden">
                <h3 class="mb-3 text-success fw-bold">RATING & REVIEWS</h3>
                            ...
                            <table class="table table-hover border">
                                ...
                                <tbody>
                                    ...
                                    while ($row = mysqli_fetch_assoc($data)) {
                                        ...
                                        $seen = '';
                                        if ($row['seen'] != 1) {
                                            $seen = "<a href='?seen=$row[rating_id]' class='btn btn-sm rounded-pill btn-success mb-2'>Mark as Read</a> <br>";
                                        }
                                        $seen .= "<a href='?del=$row[rating_id]' class='btn btn-sm rounded-pill btn-danger'>Delete</a>";
